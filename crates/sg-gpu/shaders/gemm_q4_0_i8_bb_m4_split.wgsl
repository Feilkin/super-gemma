// "BIGBOY M4 + SPLIT-M DISPATCH" int8-MMQ GEMM — FFN-down (K=21504, N=5376),
// M_TILES=4. Identical compute to bb_m4, but the M-block is chosen by a push
// constant instead of the grid, and the grid is just the N-strips: [NB_N, 1, 1].
//
// The point is concurrency control. bb_m4 launches NB_M·NB_N tiles and the
// hardware runs ~400 waves at once — enough to span TWO M-blocks' activation slabs
// (2×1.31 MiB > 2 MB L2) → L2 thrash. Here each dispatch covers ONE M-block (all
// NB_N=336 N-strips of it), so at most 336 waves exist, all sharing that M-block's
// single 1.31 MiB X-slab → it stays L2-resident. The host issues NB_M=4 such
// dispatches, serialized by an execution barrier, so only one M-block is ever in
// flight. (Trades raw occupancy 336/~400 for L2 hits — the same bargain the
// deployed kernel makes accidentally via its fat VGPR footprint.)
//
// Keeps bb_m4's levers: one-batch loads, 0-stride scale rescale, direct f16
// coopStore, no barriers (single wave). Disjoint M-blocks → the 4 dispatches write
// non-overlapping Y rows, so only an EXECUTION dependency is needed, not a memory
// one.
...
// naga 29 spells the push-constant address space `immediate`. One u32: the M-block
// this dispatch computes (m0 = mblock · M_ROWS).
struct Push { mblock: u32 }
...
fn main(
    ...
    let lid = lid_v.x;
    let m0 = push.mblock * M_ROWS;  // M-block from the push constant
    let n0 = wg.x * N_COLS;         // N-strip from the 1D grid
    let r0 = m0 * K;
    let r1 = (m0 + 16u) * K;
    let r2 = (m0 + 32u) * K;
    let r3 = (m0 + 48u) * K;
