...
/// (K, N, N-block, M-block, int8) for the supported capture targets — the FFN
/// up/down projections, int8 and f16, at their production tilings.
fn shape(kernel: &str) -> Option<(usize, usize, u32, u32, bool)> {
    Some(match kernel {
...
pub fn run(kernel: &str) -> anyhow::Result<()> {
    ...
    let groups = [n as u32 / nb, M as u32 / mb, 1];
    eprintln!(
